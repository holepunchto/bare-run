...
exports.run = async function run(bundle, opts) {
  ...
  for (const candidate of adb.list()) {
  ...
  adb.shell(device.id, `mkdir -p ${remoteDir}`, opts)
  ...
  adb.push(device.id, tmpFileName, remoteBundlePath, { stdio: 'ignore' })
  ...
  adb.push(device.id, prebuild(), remoteDir, { stdio: 'ignore'})

  const remoteDriver = path.join(remoteDir, 'bare-test-driver')
  adb.shell(device.id, `${remoteDriver} ${remoteBundlePath}`, { stdio: 'inherit' })
}
